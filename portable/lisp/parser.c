 ...
 * Создаётся объект пара с текущим элементом и рекурсивной обработкой хвоста
 *  
 ...
 * (1 2 3) (1 . (2 . (3 . nil)))
 * (1 . 2)
 * Обработка завершается при встрече правой скобки или конца ввода
 *  
 * @return указатель на объект списка
 */
object_t parse_list()
...
/**
 * Читает выражение и строит объект этого выражения
...
object_t parse()
{
    object_t el; // создается новый элемент
